...
// import ui.ChatGameView; // adjust the package if needed
...
public enum Client {
    ...
    private void error(String message) {
        ...
        private ChatGameView callback;

// public void registerCallback(ChatGameView view) {
//     this.callback = view;
// }
    ...
    private boolean processClientCommand(String text) throws IOException {
        ...
        if (text.startsWith(Constants.COMMAND_TRIGGER)) {
            ...
            else if (text.startsWith("pick")) {  // or Command.PICK.command if you have it defined
    text = text.replace("pick", "").trim();
    if (!text.matches("[rps]")) {
        LoggerUtil.INSTANCE.warning(TextFX.colorize("Invalid pick. Use /pick r, /pick p, or /pick s.", Color.RED));
        return true;
    }
    sendDoTurn(text);
    wasCommand = true;
    ...
    private void processPayload(Payload payload) {
        switch (payload.getPayloadType()) {
            ...
            break;
            case PICK: // example of picking an item in a turn
            System.out.println(payload.getMessage()); // show who picked
             break;
    ...
    /**
     * Processes a user list update payload from the server.
     * @param payload the payload containing the user list
     */
    private void processUserList(Payload payload) {
        ...
        RoomResultPayload rrp = (RoomResultPayload) payload;
        List<String> users = rrp.getRooms(); // Assuming getRooms() returns user names for USER_LIST_UPDATE
        if (users == null || users.isEmpty()) {
            LoggerUtil.INSTANCE.info(TextFX.colorize("No users found.", Color.YELLOW));
            return;
        }
        LoggerUtil.INSTANCE.info(TextFX.colorize("User List:", Color.CYAN));
        LoggerUtil.INSTANCE.info(String.join(System.lineSeparator(), users));
    }
